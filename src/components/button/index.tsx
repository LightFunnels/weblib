...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, loading, size, leftIcon, rightIcon, IconClassName, children, ...props }, ref) => {
    return (
      <button
        className={clsx(buttonVariants({ variant, size }), "flex items-center gap-2", className)}
        ref={ref}
        {...props}
      >
        {loading ? (
...
export function Spinner(props: React.HTMLAttributes<HTMLOrSVGElement>){
	return (
		<SpinnerSVG {...props} className={clsx("lfui-spinner_primary", props.className)} />
	)
}
...
export function SpinnerSVG(props: React.HTMLAttributes<HTMLOrSVGElement>){
	return (
		<svg viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" {...props} className={clsx("lfui-spinner", props.className)} >
